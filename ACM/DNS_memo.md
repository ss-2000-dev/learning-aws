...
| **CNAME レコード** | 別名定義。「この名前は別の名前の別名です」 |
| **TXT レコード** | 任意の文字列を保存するレコード |
| **MX レコード** | メールサーバ設定 |
| **NS レコード** | そのドメインを管理するネームサーバの情報 |

さらに詳しく
| レコード | 役割 | 具体例（名前） | 具体例（値） |
| --------- | ------------------------ | ----------------------------------------- | -------------------------------------- |
...
| **TXT** | 任意文字列 | example.com | "v=spf1 include:\_spf.google.com ~all" |
| **MX** | メールサーバ | example.com | 10 mail.example.com |
| **NS** | 権威ネームサーバ | example.com | ns1.example-dns.com |

## DNS レコード名の由来

- DNS は 1980 年代に設計された古いプロトコルで、名称は当時の議論や RFC（仕様書）に基づき決められています。由来は以下のとおり。
